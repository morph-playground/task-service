...
export class TaskService {
  ...
  async createTask(userId: string, createTaskRequest: CreateTaskRequest): Promise<Task> {
    console.debug(`[TaskService] Checking CREATE permission for user: ${userId}`);
    const hasPermission = await this.permissionServiceClient.hasPermission(userId, Domain.TASK, Action.CREATE);
    if (!hasPermission) {
      console.debug(`[TaskService] User ${userId} lacks permission to create task`);
    ...
    console.debug(`[TaskService] Created task with id: ${task.id} for user: ${userId}`);
    return task;
  }

  async getTasks(userId: string): Promise<Task[]> {
    console.debug(`[TaskService] Checking LIST permission for user: ${userId}`);
    const hasPermission = await this.permissionServiceClient.hasPermission(userId, Domain.TASK, Action.LIST);
    if (!hasPermission) {
      console.debug(`[TaskService] User ${userId} lacks permission to list tasks`);
      throw new Error('Insufficient permissions to list tasks');
    }

    const userTasks = this.tasks.filter(task => task.userId === userId);
    console.debug(`[TaskService] Found ${userTasks.length} tasks for user: ${userId}`);
    return userTasks;
  }
}